...
import { getFutureWeekendDetail } from "@/lib/data";
...
export async function GET(request: NextRequest) {
  ...
  if (!w1 || !w2) {
    return NextResponse.json({ error: "Both w1 and w2 are required" }, { status: 400 });
  }
  ...
  const majorStudiosOnly = searchParams.get("studios") === "major";
  const weekendA = getFutureWeekendDetail(parsed1.year, parsed1.week, { majorStudiosOnly });
  const weekendB = getFutureWeekendDetail(parsed2.year, parsed2.week, { majorStudiosOnly });

  if (!weekendA || !weekendB) {
    return NextResponse.json({ error: "One or both weekends not found" }, { status: 404 });
  }
